...
test_that("Covariance is computed correctly", {
  ...
  expect_that(cov_X,           is_equivalent_to(crosscovArma(X, X, 0)))
  expect_that(cov_X*(n - 1)/n, is_equivalent_to(crosscovArma(X, X, 1)))
...
test_that("Cross-correlation is computed correctly", {
  expect_that(cov_XY,           is_equivalent_to(crosscovArma(X, Y, 0)))
